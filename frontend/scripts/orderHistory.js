async function fetchOrdersPending() {
  ...
  if (token) {
    ...
    alert("You must be signed in to view order history.");
    window.location.href = "./signin.html";
    return;
  }
  const res = await fetch(
    `http://localhost:8000/order?status=Pending&user_id=${payload.id}`,
    {
      method: "GET",
    }
  );
  ...
  const container = document.getElementById("items-pending");
  if (!container) {
...
async function fetchOrdersCompleted() {
  ...
  const res = await fetch(
    `http://localhost:8000/order?status=Completed&user_id=${payload.id}`,
    {
      method: "GET",
    }
  );
  ...
  const container = document.getElementById("items-completed");
  if (!container) {
    console.error("Order list container not found");
    return;
  }
